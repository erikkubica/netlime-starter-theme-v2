...
elseif (!comments_open() && !is_page() && post_type_supports(get_post_type(), 'comments')) :
    ?>
    <p><?php echo __('Comments are closed', 'theme') ?></p>
<?php endif; ?>
...
$comments_arg = array(
    ...
    'fields' => apply_filters('comment_form_default_fields', array(
        'autor' => '<div class="form-group">' . '<label for="author">' . __('Name', 'theme') . '</label> ' . ($req ? '<span>*</span>' : '') .
            '<input id="author" name="author" class="form-control" type="text" value="" size="30"' . $aria_req . ' />' .
            '<p id="d1" class="text-danger"></p>' . '</div>',
        'email' => '<div class="form-group">' . '<label for="email">' . __('Email', 'theme') . '</label> ' . ($req ? '<span>*</span>' : '') .
            '<input id="email" name="email" class="form-control" type="text" value="" size="30"' . $aria_req . ' />' .
            '<p id="d2" class="text-danger"></p>' . '</div>',
        'url' => '')),
    'comment_field' => '<div class="form-group">' . '<label for="comment">' . __('Comment', 'theme') . '</label><span>*</span>' .
...
<script>
	...
	function validateForm() {
		...
		if (x == null || x == "") {
			d1.innerHTML = "<?php echo __('Name is required', 'theme'); ?>";
			z = false;
		} else {
			d1.innerHTML = "";
		}
		...
		if (y == null || y == "") {
			d2.innerHTML = "<?php echo __('Email is required', 'theme'); ?>";
			z = false;
		} else {
			d2.innerHTML = "";
		}
		...
		if (z == null || z == "") {
			d3.innerHTML = "<?php echo __('Comment is required', 'theme'); ?>";
			z = false;
		} else {
			d3.innerHTML = "";
		}
